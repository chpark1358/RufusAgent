...
import { appendStructuredLog } from "../../rufus-agent/internal/core/rufus-agent-log.mjs";
import { resolveRunPromptPath } from "../../rufus-agent/internal/core/rufus-agent-runtime-context.mjs";
import { defaultDbPath, openStore } from "../../rufus-agent/internal/core/rufus-agent-store.mjs";
...
export function storeForArgs(args) {
  return openStore(args.dbPath ?? defaultDbPath(), {
    workspaceRoot: args.workspaceRoot ?? process.env.RUFUS_AGENT_WORKSPACE_ROOT ?? null,
    startDir: args.workspaceRoot ?? process.env.RUFUS_AGENT_WORKSPACE_ROOT ?? process.cwd()
  });
}
...
export function validateCurrentRunBinding(store, args) {
  ...
  if (args.agentId) {
    if (args.agentId === "agent_main") {
      throw new Error("agent_main must report through report-main.mjs. Do not use the generic rufus-agent-runtime report-* scripts for the Main agent.");
    }
...
export function logReportLifecycle(stage, scriptName, details = {}) {
  return appendStructuredLog("rufus-agent-runtime", {
