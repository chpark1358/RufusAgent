...
- RufusAgent v1 runtime supports one active `task_id` per agent at a time
...
- if the planner driver is remote, runtime reporting still writes to the local
  RufusAgent DB unless a different transport is explicitly introduced
...
- if a non-Main agent becomes ready to start and still emits no runtime event
  for about 90 seconds, RufusAgent may auto-mark it as `blocked`
...
- `read_paths`, `write_paths`, and `deny_paths` are RufusAgent policy metadata unless
