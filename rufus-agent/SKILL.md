...
RufusAgent runtime helper surface.
...
6. After dashboard approval, the operator only needs to type `승인` in the
   current RufusAgent terminal session.
...
8.6. If the manifest packet includes `assistant_launch_recipe.tool_name` and
   ...
   Do not overclaim that this makes every RufusAgent policy field natively enforced
   by the host launch surface.
9. Require every sub-agent to follow the RufusAgent runtime reporting rules.
...
11.3. Prefer `assistant_launch_recipe.tool_args` when present. That field is
    ...
    actually enforce. Sandbox, MCP, helper-skill, and path rules remain RufusAgent
...
14. While and only while this skill is active, the Main Agent should report its
    own progress through the rufusagent wrapper command instead of ad-hoc prose.

## Approval Guardrails

- A user asking to "use RufusAgent", "work in parallel", "research and
...
- path policies, ownership, and merge order are RufusAgent coordination rules, not
  native Codex ACL guarantees
- in manifest-only launch recipes, `fork_context=false` is a hard RufusAgent launch
  invariant
...
- a public low-level `spawn/list/wait/terminate/send_input` API for Codex
  sub-agents
- recursive child spawning by RufusAgent sub-agents in v1
...
When RufusAgent is distributed as a public community edition, keep the authority
split explicit:
...
- use a hosted normalization/binding service for turning that creative draft into an executable RufusAgent plan
...
- Standard public/community install:
  ...
  - RufusAgent defaults to `https://agent.zooo.kr/v1/planner/draft`
  - RufusAgent defaults to `https://agent.zooo.kr/v1/planner/ticket`
  - RufusAgent defaults to `https://agent.zooo.kr/rufus-agent-dashboard/`
- Optional private or self-hosted override:
  - `RUFUS_AGENT_PLANNER_DRIVER=remote_http`
  - `RUFUS_AGENT_REMOTE_PLAN_NORMALIZER_URL=https://your-planner.example/v1/planner/draft`
  - `RUFUS_AGENT_REMOTE_PLAN_NORMALIZER_TICKET_URL=https://your-planner.example/v1/planner/ticket`
  - `RUFUS_AGENT_REMOTE_PLAN_NORMALIZER_BEARER_TOKEN=<token>`
  - `RUFUS_AGENT_REMOTE_DASHBOARD_BASE_URL=https://your-planner.example/rufus-agent-dashboard/`
    - optional: if omitted, RufusAgent derives `/rufus-agent-dashboard/` from the hosted planner origin
...
- if a custom remote normalization override is incomplete, RufusAgent fails fast instead of generating a local fallback draft
...
- The canonical SQLite event table is `events`. A compatibility view named
  ...
  prefer the RufusAgent APIs and current table name.
...
- `provider_refs` must describe what RufusAgent actually knows from the provider
...
- `rufusagent multi-agent 스킬로 진행` means to enter this workflow, not to approve a
...
- Set `RUFUS_AGENT_INSTALL_ROOT` to your RufusAgent install root on another machine before using the commands below.

- Recommend and save a plan:
  - `node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/recommend-plan.mjs --creative-draft-file /abs/draft.json --request-summary "request summary"`
  - Optional workspace override: `node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/recommend-plan.mjs --creative-draft-file /abs/draft.json --workspace-root /abs/workspace --db /abs/rufusagent.sqlite --request-summary "request summary"`
  - Public hosted normalizer default: `node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/recommend-plan.mjs --creative-draft-file /abs/draft.json --workspace-root /abs/workspace --db /abs/rufusagent.sqlite --request-summary "request summary"`
  - Private/self-hosted override: `RUFUS_AGENT_PLANNER_DRIVER=remote_http RUFUS_AGENT_REMOTE_PLAN_NORMALIZER_URL=https://your-planner.example/v1/planner/draft RUFUS_AGENT_REMOTE_PLAN_NORMALIZER_TICKET_URL=https://your-planner.example/v1/planner/ticket RUFUS_AGENT_REMOTE_PLAN_NORMALIZER_BEARER_TOKEN=<token> node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/recommend-plan.mjs --creative-draft-file /abs/draft.json --workspace-root /abs/workspace --db /abs/rufusagent.sqlite --request-summary "request summary"`
...
- Present the user-facing proposal:
  - `node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/present-proposal.mjs --creative-draft-file /abs/draft.json --request-summary "request summary"`
  - Optional workspace override: `node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/present-proposal.mjs --creative-draft-file /abs/draft.json --workspace-root /abs/workspace --db /abs/rufusagent.sqlite --request-summary "request summary"`
  - Optional dashboard bridge override: `node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/present-proposal.mjs --creative-draft-file /abs/draft.json --workspace-root /abs/workspace --db /abs/rufusagent.sqlite --dashboard-url http://127.0.0.1:18081 --request-summary "request summary"`
  - Force planner driver only when debugging: `RUFUS_AGENT_PLANNER_DRIVER=remote_http`
  - If the draft is invalid, RufusAgent fails before planner lock and DB mutation.
...
- Internal terminal confirmation helper used after the user types `승인`:
  - `node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/confirm-plan.mjs --workspace-root /abs/workspace`
  - Claude Code override: `node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/confirm-plan.mjs --workspace-root /abs/workspace --adapter-type claude-code-subagent --adapter-backend claude-code-manual`
  - Optional explicit binding: `node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/confirm-plan.mjs --workspace-root /abs/workspace --db /abs/rufusagent.sqlite --dashboard-url http://127.0.0.1:18081`
- Show the authoritative workspace binding before ad-hoc diagnostics:
  - `node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/show-authority.mjs --workspace-root /abs/workspace`
- Show the launch manifest prepared by the active adapter:
  - `node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/show-run-launch-manifest.mjs --run-id <run_id> --db <db_path>`
...
- Create a run directly only for single-agent or internal testing paths:
  - `node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/create-run.mjs --plan-id <plan_id> --mode dry-run --db <db_path>`
- Build a sub-agent prompt packet:
  - `node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/build-subagent-prompt.mjs --run-id <run_id> --agent-id <agent_id> --db <db_path>`
- Build a plain-text delegation prompt for an actual provider sub-agent:
  - `node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/build-subagent-prompt.mjs --run-id <run_id> --agent-id <agent_id> --format text --db <db_path>`
...
- Retry a run:
  - `node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/retry-run.mjs --run-id <run_id> --db <db_path>`
  - Multi-agent retry is disabled. For multi-agent plans, re-approve in the dashboard and then type `승인` in the current RufusAgent terminal session.
- Change run status:
  - `node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/set-run-status.mjs --run-id <run_id> --status cancelled --db <db_path>`
  - Do not use this helper to revive a finished run. Terminal run finality must be preserved.
- Start dashboard:
  - `node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/start-dashboard.mjs`
  - Optional workspace override: `node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/start-dashboard.mjs --workspace-root /abs/workspace --db /abs/rufusagent.sqlite`
  - Optional dashboard override: `node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/start-dashboard.mjs --workspace-root /abs/workspace --db /abs/rufusagent.sqlite --dashboard-url http://127.0.0.1:18081`
...
- Export a self-contained two-skill portable bundle:
  - `node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/export-portable-bundle.mjs`
  - Optional destination override: `node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/export-portable-bundle.mjs --output-dir /abs/output --bundle-name rufus-agent-portable`
- Export a GitHub/public release with only the two skill folders:
  ...
  - Optional destination override: `node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/export-public-release.mjs --output-dir /abs/output --bundle-name rufus-agent-public`
  - Validate an exported release root: `node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/check-public-release.mjs --release-root /abs/output/rufus-agent-public`
- Inspect stored snapshot:
  - `node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/show-snapshot.mjs --db <db_path>`
...
- Generate compact runtime context files for a run:
  - `node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/prepare-run-context.mjs --run-id <run_id> --db <db_path>`
- Report Main Agent progress for the active rufusagent run:
  - `node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/report-main.mjs --db <db_path> --type progress --message "최종 정리 시작" --detail "설계와 구현 결과를 함께 검토하고 있습니다. 충돌 지점을 먼저 정리합니다."`
- Report Main Agent completion for the active rufusagent run:
  - `node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/report-main.mjs --db <db_path> --type completion --result success --summary "최종 정리 완료" --detail "구현과 검증 결과를 하나로 묶었습니다. 이제 사용자에게 결과를 설명할 수 있습니다."`
- Show run diagnostics and recent runtime logs:
  - `node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/show-run-diagnostics.mjs --run-id <run_id> --db <db_path>`
...
- When comparing RufusAgent against other providers, rely on the bundled references in this skill and any local notes you intentionally ship alongside the portable bundle or public release.
- For moving RufusAgent to another computer, read [portable-setup.md](references/portable-setup.md).

## Coordination With rufus-agent-runtime

When spawning or instructing sub-agents, explicitly require them to follow the
current RufusAgent runtime reporting contract and use the generated runtime context.
...
This Main Agent reporting rule applies only while `rufus-agent` is the
...
the log directory resolved by RufusAgent at runtime, typically `<data_dir>/logs/rufus-agent-runtime-YYYY-MM-DD.jsonl`.

## Dashboard Binding Notes

- The default dashboard URL is now workspace-scoped. RufusAgent derives a stable local port from `workspace_root` instead of assuming one global fixed port.
...
- Avoid exporting one global `RUFUS_AGENT_DASHBOARD_URL` or `RUFUS_AGENT_DASHBOARD_PORT` across multiple workspaces unless you intentionally want every workspace to share that override.
