...
The public/community edition of RufusAgent ships only the local client pieces:
...
Standard public/community installs use the hosted RufusAgent planner service at:

- `https://agent.zooo.kr/v1/planner/draft`
- `https://agent.zooo.kr/v1/planner/ticket`
- `https://agent.zooo.kr/rufus-agent-dashboard/`
...
- `RUFUS_AGENT_REMOTE_DASHBOARD_BASE_URL=https://agent.example/rufus-agent-dashboard/`

`RUFUS_AGENT_REMOTE_PLAN_NORMALIZER_BEARER_TOKEN` is optional in the public default path and is only needed when the hosted service you target requires bearer authentication in addition to ticket flow.
...
draft into an executable RufusAgent plan. It must not author the initial draft and
