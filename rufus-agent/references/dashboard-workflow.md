...
- The review URL is a same-computer session link. It depends on the local
  loopback bridge and the current RufusAgent terminal session, so it must not be
...
- RufusAgent also writes workspace-local authority metadata to
  `<workspace_root>/.rufusagent/authority.json` and runtime authority metadata to
...
- After dashboard approval, the dashboard must tell the operator to type `승인` in the current RufusAgent terminal session.
...
- The dashboard may edit supported sub-agent controls, but only within the
  current adapter control surface and RufusAgent-local governance fields.
