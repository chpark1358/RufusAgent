...
import { rufusagentInstallRoot, detectWorkspaceRoot, normalizeWorkspacePath } from "../internal/core/rufus-agent-runtime-paths.mjs";
...
function buildGuidance(errorCode, message, extra = {}) {
  return {
    ...
    expected_usage: [
      ...
      "node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/export-portable-bundle.mjs --output-dir /abs/output",
      "node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/export-portable-bundle.mjs --output-dir /abs/output --bundle-name rufus-agent-portable",
      "node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/export-portable-bundle.mjs --output-dir /abs/output --workspace-root /abs/workspace",
      "node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/export-portable-bundle.mjs --output-dir /abs/output --force --skip-validate"
    ],
    rules: [
...
const args = {
  ...
  bundleName: `rufus-agent-portable-bundle-${timestampLabel()}`,
  workspaceRoot: process.env.RUFUS_AGENT_WORKSPACE_ROOT
    ? normalizeWorkspacePath(process.env.RUFUS_AGENT_WORKSPACE_ROOT)
...
const installRoot = process.env.RUFUS_AGENT_INSTALL_ROOT
  ? resolve(process.env.RUFUS_AGENT_INSTALL_ROOT)
  : rufusagentInstallRoot();
const sourceSkillsRoot = resolve(installRoot, "skills");
const sourceMultiAgentRoot = resolve(sourceSkillsRoot, "rufus-agent");
const sourceRuntimeRoot = resolve(sourceSkillsRoot, "rufus-agent-runtime");
...
const exportedMultiAgentRoot = resolve(bundleSkillsRoot, "rufus-agent");
const exportedRuntimeRoot = resolve(bundleSkillsRoot, "rufus-agent-runtime");
...
const readme = `# RufusAgent Portable Bundle

This bundle contains a self-contained RufusAgent runtime package with only:
...
2. Export \`RUFUS_AGENT_INSTALL_ROOT\` to this bundle root.
3. Read \`skills/rufus-agent/references/portable-setup.md\`.
4. Run:
...
- Bundled runtime deps under \`skills/rufus-agent/node_modules\`
...
const manifest = {
  ...
  dashboard_dist: "skills/rufus-agent/internal/dashboard/dist",
  portable_setup_excerpt: setupDocText.split("\n").slice(0, 24).join("\n")
};

writeFileSync(resolve(bundleRoot, "README.md"), readme, "utf8");
writeFileSync(resolve(bundleRoot, "RUFUS_AGENT_PORTABLE_BUNDLE.json"), `${JSON.stringify(manifest, null, 2)}\n`, "utf8");
...
if (args.validate) {
  const run = spawnSync(process.execPath, [exportedSmokeTest], {
    ...
    env: {
      ...process.env,
      RUFUS_AGENT_INSTALL_ROOT: bundleRoot,
      RUFUS_AGENT_WORKSPACE_ROOT: args.workspaceRoot
    },
    encoding: "utf8"
  });
  let parsed = null;
...
const result = {
  ...
  files_written: [
    ...
    resolve(bundleRoot, "RUFUS_AGENT_PORTABLE_BUNDLE.json")
  ],
  copied_skills: [
