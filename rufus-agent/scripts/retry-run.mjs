...
import { launchRunWithAdapter } from "../internal/core/rufus-agent-adapters.mjs";
import { createRunSnapshot } from "../internal/core/rufus-agent-run-snapshot.mjs";
import { resolveMainProviderSessionIdentity } from "../internal/core/rufus-agent-provider-session.mjs";
import { defaultDbPath, openStore } from "../internal/core/rufus-agent-store.mjs";
import { isStructurallyMultiAgentPlan } from "../internal/core/rufus-agent-validation.mjs";
...
if (isStructurallyMultiAgentPlan(plan)) {
  console.error("Multi-agent retry is disabled. Re-approve the plan in the dashboard, then type `승인` in the current RufusAgent terminal session.");
  process.exit(1);
}
