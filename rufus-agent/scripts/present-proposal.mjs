...
import { recommendPlanAutonomous } from "../internal/core/rufus-agent-autonomous-planner.mjs";
import { openStore } from "../internal/core/rufus-agent-store.mjs";
import { ensureDashboardReady } from "../internal/core/rufus-agent-dashboard-launcher.mjs";
import { acquirePlannerLock } from "../internal/core/rufus-agent-planner-lock.mjs";
import { localize } from "../internal/core/rufus-agent-language.mjs";
import { resolvePlannerConfig, summarizePlannerConfig, validatePlannerConfig } from "../internal/core/rufus-agent-planner-driver.mjs";
import { creativeDraftGuidance, loadCreativeDraft } from "../internal/core/rufus-agent-creative-draft.mjs";
import { ensureDashboardBridgeToken, operatorDashboardUrlForWorkspace } from "../internal/core/rufus-agent-dashboard-bridge.mjs";
import { writeDashboardAuthorityArtifacts } from "../internal/core/rufus-agent-authority-artifacts.mjs";
import { formatAuthorityMismatchMessage, resolveCliAuthoritativeBinding } from "../internal/core/rufus-agent-cli-authority.mjs";
import { dashboardUrlForWorkspace } from "../internal/core/rufus-agent-runtime-paths.mjs";
import { validateRequestSummaryInput } from "../internal/core/rufus-agent-validation.mjs";
...
const args = {
  dbPath: null,
  dashboardUrl: null,
  creativeDraftFile: process.env.RUFUS_AGENT_CREATIVE_DRAFT_FILE ?? null,
  creativeDraftBase64: process.env.RUFUS_AGENT_CREATIVE_DRAFT_BASE64 ?? null,
  creativeDraftJson: process.env.RUFUS_AGENT_CREATIVE_DRAFT_JSON ?? null,
  save: true,
  workspaceRoot: process.env.RUFUS_AGENT_WORKSPACE_ROOT ? resolve(process.env.RUFUS_AGENT_WORKSPACE_ROOT) : process.cwd(),
  timeoutMs: 180000,
...
function dashboardGuidanceLines(plan, dashboardState) {
  ...
  const sameComputerLine = localize(
    language,
    "이 검토 링크는 같은 컴퓨터의 현재 RufusAgent 세션용입니다. 다른 컴퓨터에서는 열리지 않습니다.",
    "This review link only works on the same computer while the current RufusAgent session and local bridge are alive."
  );
...
function buildGuidance(errorCode, message, extra = {}) {
  return {
    ...
    message,
    expected_usage: [
      "node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/present-proposal.mjs --creative-draft-file /abs/draft.json --workspace-root /abs/workspace --request-summary \"request summary\"",
      "node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/present-proposal.mjs --creative-draft-json '{\"plan_title\":\"Short title\",\"preferred_language\":\"ko\",\"single_or_multi\":\"multi\",\"multi_agent_beneficial\":true,\"recommendation_summary\":\"...\",\"recommendation_reasons\":[\"...\"],\"subagents\":[...]}' --request-summary \"request summary\"",
      "node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/present-proposal.mjs --creative-draft-file /abs/draft.json --language ko --workspace-root /abs/workspace --db /path/to/rufusagent.sqlite --timeout-ms 180000 --request-summary \"request summary\"",
      "node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/present-proposal.mjs --creative-draft-file /abs/draft.json --workspace-root /abs/workspace --db /path/to/rufusagent.sqlite --dashboard-url http://127.0.0.1:18081 --request-summary \"request summary\""
    ],
    rules: [
      "--request-summary is required",
      "A local creative draft is required via --creative-draft-file, --creative-draft-base64, --creative-draft-json, or RUFUS_AGENT_CREATIVE_DRAFT_FILE",
      ...
      "Set RUFUS_AGENT_REMOTE_PLAN_NORMALIZER_URL and RUFUS_AGENT_REMOTE_PLAN_NORMALIZER_TICKET_URL only when overriding the default hosted planner",
      "RUFUS_AGENT_REMOTE_PLAN_NORMALIZER_BEARER_TOKEN is optional and is only needed for restricted private deployments",
...
process.stdout.write(
  [
    ...
    `${localize(plan.preferred_language, "기준 DB", "Authoritative DB")}: ${plan.authoritative_db_path}`,
    ...dashboardGuidanceLines(plan, dashboardState),
    localize(plan.preferred_language, "먼저 대시보드에서 승인하세요. 그다음 현재 RufusAgent 터미널 세션에 승인이라고 입력하면 최신 대시보드 상태 기준으로 확정됩니다.", "Approve in the dashboard first. Then type `승인` in the current RufusAgent terminal session to confirm using the latest dashboard state."),
    "",
    JSON.stringify(plan, null, 2)
  ].join("\n")
);
