...
import { appendStructuredLog } from "../internal/core/rufus-agent-log.mjs";
import { defaultDbPath, openStore } from "../internal/core/rufus-agent-store.mjs";
import { compactList, compactText, createEventId, nowIso, runWithRetry } from "../../rufus-agent-runtime/scripts/_report-common.mjs";

const args = {
  dbPath: null,
  type: "progress",
  workspaceRoot: process.env.RUFUS_AGENT_WORKSPACE_ROOT ? resolve(process.env.RUFUS_AGENT_WORKSPACE_ROOT) : process.cwd(),
  workspaceRootExplicit: Boolean(process.env.RUFUS_AGENT_WORKSPACE_ROOT)
};
...
function findLatestActiveRufusAgentRun(store) {
  return store
    .listRuns()
...
function findCodexSessionTranscriptPath(threadId, explicitPath = null) {
  const rootDir = resolve(process.env.RUFUS_AGENT_CODEX_SESSIONS_ROOT ?? resolve(homedir(), ".codex", "sessions"));
...
function findLatestClaudeSessionTranscriptForWorkspace(workspaceRoot) {
  ...
  const claudeProjectsRoot = resolve(process.env.RUFUS_AGENT_CLAUDE_PROJECTS_ROOT ?? resolve(homedir(), ".claude", "projects"));
...
function extractRunIdFromPrompt(prompt) {
  ...
  const introMatch = text.match(/for RufusAgent run ([A-Za-z0-9_:-]+)/);
  return introMatch?.[1] ?? null;
}
...
function inferProviderSessionKind(run, args) {
  ...
  if (args.providerSessionFile) {
    const claudeProjectsRoot = normalizePath(resolve(process.env.RUFUS_AGENT_CLAUDE_PROJECTS_ROOT ?? resolve(homedir(), ".claude", "projects")));
  ...
  if (args.providerSessionThreadId || process.env.RUFUS_AGENT_MAIN_PROVIDER_SESSION_THREAD_ID || process.env.CODEX_THREAD_ID) {
    return "codex";
  }
  return "unknown";
}
...
try {
  const run = runWithRetry("report-main-resolve-run", () => {
    ...
    try {
      return args.runId ? store.getRun(args.runId) : findLatestActiveRufusAgentRun(store);
    } finally {
      store.close();
    }
  });
  if (!run) {
    throw new Error("No active rufusagent run found. If terminal confirmation just prepared a manifest-only run, pass --run-id explicitly or use the generated run-scoped prompt/command files.");
  }
  ...
  const providerThreadId = providerSessionKind === "codex"
    ? (args.providerSessionThreadId ?? process.env.RUFUS_AGENT_MAIN_PROVIDER_SESSION_THREAD_ID ?? process.env.CODEX_THREAD_ID ?? null)
    : null;
  ...
  appendStructuredLog("rufus-agent-runtime", {
