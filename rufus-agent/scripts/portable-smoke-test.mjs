...
import { detectWorkspaceRoot } from "../internal/core/rufus-agent-runtime-paths.mjs";

const SKILL_ROOT = resolve(fileURLToPath(new URL("..", import.meta.url)));
const SKILLS_ROOT = resolve(SKILL_ROOT, "..");
const RUNTIME_SKILL_ROOT = resolve(SKILLS_ROOT, "rufus-agent-runtime");
...
const workspaceRoot = process.env.RUFUS_AGENT_WORKSPACE_ROOT ?? detectWorkspaceRoot(process.cwd());
const expectedWorkspaceDbPath = resolve(workspaceRoot, ".rufusagent", "data", "rufus-agent.sqlite");
const expectedWorkspaceRuntimeRoot = resolve(workspaceRoot, ".rufusagent", "runtime");
const result = {
  ok: true,
  node_version: process.version,
  workspace_root: workspaceRoot,
  resolved_db_path: process.env.RUFUS_AGENT_DB_PATH ?? expectedWorkspaceDbPath,
  resolved_runtime_root: process.env.RUFUS_AGENT_RUNTIME_ROOT ?? expectedWorkspaceRuntimeRoot,
  checks: []
};
...
pushCheck("node_version", parseMajor(process.version) >= 22, "Node.js >= 22 is required because RufusAgent uses node:sqlite.");
...
pushCheck(
  ...
  existsSync(expectedWorkspaceDbPath)
    ...
    : `Workspace-local DB not present at ${expectedWorkspaceDbPath}; use --workspace-root or RUFUS_AGENT_WORKSPACE_ROOT if you expected workspace-local binding.`
);
pushCheck(
  ...
  existsSync(expectedWorkspaceRuntimeRoot)
    ...
    : `Workspace-local runtime root not present at ${expectedWorkspaceRuntimeRoot}; use --workspace-root or RUFUS_AGENT_WORKSPACE_ROOT if you expected workspace-local binding.`
);
pushCheck(
