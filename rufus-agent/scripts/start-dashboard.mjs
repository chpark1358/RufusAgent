...
import { dashboardPortForWorkspace, dashboardUrlForWorkspace } from "../internal/core/rufus-agent-runtime-paths.mjs";
import { openStore } from "../internal/core/rufus-agent-store.mjs";

const args = {
  workspaceRoot: process.env.RUFUS_AGENT_WORKSPACE_ROOT ?? process.cwd(),
...
function buildGuidance(errorCode, message, extra = {}) {
  return {
    ...
    expected_usage: [
      ...
      "node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/start-dashboard.mjs --workspace-root /abs/workspace",
      "node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/start-dashboard.mjs --workspace-root /abs/workspace --db /abs/rufusagent.sqlite",
      "node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/start-dashboard.mjs --workspace-root /abs/workspace --dashboard-url http://127.0.0.1:18081"
    ],
    rules: [
...
if (hasExplicitWorkspaceRoot) {
  if (binding.workspace_root) {
    process.env.RUFUS_AGENT_WORKSPACE_ROOT = binding.workspace_root;
  } else {
    delete process.env.RUFUS_AGENT_WORKSPACE_ROOT;
  }
} else {
  if (binding.workspace_root) {
    process.env.RUFUS_AGENT_WORKSPACE_ROOT ??= binding.workspace_root;
...
if (hasExplicitDbPath) {
  process.env.RUFUS_AGENT_DB_PATH = binding.db_path;
} else {
  process.env.RUFUS_AGENT_DB_PATH ??= binding.db_path;
}
...
process.env.RUFUS_AGENT_RUNTIME_ROOT = binding.runtime_root;
process.env.RUFUS_AGENT_INSTALL_ROOT ??= binding.install_root;
process.env.RUFUS_AGENT_DASHBOARD_URL = dashboardUrl;
process.env.RUFUS_AGENT_DASHBOARD_PORT = String(dashboardPort);
