...
import { buildAgentPacketForRun, buildLaunchManifestForRun, cancelRunWithAdapter, collectAdapterStatusForRun, defaultAdapterConfig, getAdapterCapabilities, launchRunWithAdapter, listAdapters, prepareRunContextForRun } from "../core/rufus-agent-adapters.mjs";
import { resolveRufusAgentBinding, sanitizedBinding } from "../core/rufus-agent-binding-resolver.mjs";
import {
  ensureDashboardBridgeToken,
  operatorDashboardUrlForWorkspace,
  readDashboardBridgeTokenRecord,
  remoteDashboardOrigin
} from "../core/rufus-agent-dashboard-bridge.mjs";
import { collectRunDiagnostics } from "../core/rufus-agent-diagnostics.mjs";
import { localize, normalizePreferredLanguage, SUPPORTED_LANGUAGES } from "../core/rufus-agent-language.mjs";
import { resolvePlannerConfig, summarizePlannerConfig, validatePlannerConfig } from "../core/rufus-agent-planner-driver.mjs";
import { dashboardHost, dashboardPortForWorkspace, dashboardUrlForWorkspace, defaultLogDir } from "../core/rufus-agent-runtime-paths.mjs";
import { openStore, openStoreWithOptions } from "../core/rufus-agent-store.mjs";
import { createRunSnapshot } from "../core/rufus-agent-run-snapshot.mjs";
import { isStructurallyMultiAgentPlan, validatePlan, validatePlanForAdapter, validateRequestSummaryInput } from "../core/rufus-agent-validation.mjs";

const ROOT_DIR = resolve(fileURLToPath(new URL("../../../..", import.meta.url)));
const FRONTEND_DIST_DIR = resolve(fileURLToPath(new URL("./dist", import.meta.url)));
const BINDING = resolveRufusAgentBinding({
  workspaceRoot: process.env.RUFUS_AGENT_WORKSPACE_ROOT ?? null,
  dbPath: process.env.RUFUS_AGENT_DB_PATH ?? null,
  runtimeRoot: process.env.RUFUS_AGENT_RUNTIME_ROOT ?? null,
...
const RUFUS_AGENT_ENFORCED_FIELDS = [
  "depends_on",
  "reporting_contract",
...
const DASHBOARD_LOG_PATH = resolve(
  ...
  `rufus-agent-dashboard-${new Date().toISOString().slice(0, 10)}.jsonl`
);
...
function shouldIgnoreWorkspaceArtifactPath(relPath) {
  ...
  return normalized.startsWith(".rufusagent_fix/");
}
...
if (!isFile(resolve(FRONTEND_DIST_DIR, "index.html"))) {
  console.warn(`rufusagent dashboard dist is missing at ${FRONTEND_DIST_DIR}. Restore the bundled dist assets under rufus-agent/internal/dashboard/dist.`);
}
...
function launchPlannerWorker(jobId, preferredLanguage) {
  ...
  const workerScript = resolve(ROOT_DIR, "skills", "rufus-agent", "scripts", "run-planner-job.mjs");
  const child = spawn(
    process.execPath,
    [
      workerScript,
    ...
    ],
    {
      ...
      env: {
        ...process.env,
        RUFUS_AGENT_DB_PATH: BINDING.db_path,
        RUFUS_AGENT_WORKSPACE_ROOT: SERVER_WORKSPACE_ROOT,
        RUFUS_AGENT_RUNTIME_ROOT: BINDING.runtime_root,
        RUFUS_AGENT_INSTALL_ROOT: BINDING.install_root,
        RUFUS_AGENT_DASHBOARD_URL: DASHBOARD_URL,
        RUFUS_AGENT_PREFERRED_LANGUAGE: preferredLanguage,
        RUFUS_AGENT_PLANNER_DRIVER: plannerConfig.planner_driver
      }
    }
  );
...
function applyBridgeCors(request, response) {
  ...
  response.setHeader("Access-Control-Allow-Headers", "Content-Type, X-RufusAgent-Bridge-Token");
...
function bindingStillValid() {
  ...
  try {
    const currentBinding = resolveRufusAgentBinding({
...
function healthPayload() {
  ...
  return {
    ...
    control_matrix: {
      ...
      rufusagent_enforced_fields: RUFUS_AGENT_ENFORCED_FIELDS,
...
function bridgeTokenIsValid(request) {
  ...
  return Boolean(expected) && String(request.headers["x-rufus-agent-bridge-token"] ?? "") === expected;
}
...
function bridgeTokenFromRequestUrl(rawUrl = "") {
  try {
    ...
    return String(
      ...
      ?? parsed.searchParams.get("rufusagent_bridge_token")
      ?? ""
    );
  } catch {
    return "";
...
function summarizeAgentControls(agent) {
  ...
  const rufusagentEnforced = RUFUS_AGENT_ENFORCED_FIELDS.filter((field) => hasConfiguredValue(agent[field]));
  ...
  return {
    ...
    rufusagent_enforced: rufusagentEnforced,
    advisory,
...
function computeOperatorAction(plan, run) {
  ...
  if (plan.terminal_confirmation_status !== "confirmed") {
    return {
      ...
      next_action: localize(preferredLanguage, "현재 RufusAgent 터미널 세션에 승인이라고 입력하세요.", "Type 승인 in the current RufusAgent terminal session."),
      resume_supported: false,
      reapproval_required: false,
      can_approve: false,
      can_retry: false,
  ...
  if (run.status === "failed" || run.status === "cancelled" || run.status === "stale") {
    return {
      ...
      next_action: localize(preferredLanguage, "대시보드에서 계획을 다시 승인한 뒤 현재 RufusAgent 터미널 세션에 승인이라고 입력하세요.", "Re-approve the plan in the dashboard, then type 승인 in the current RufusAgent terminal session."),
      resume_supported: false,
  ...
  try {
    ...
    if (request.method === "POST" && url.pathname === "/api/plans/recommend") {
      ...
      sendJson(response, 400, {
        ...
        expected_usage: {
          present_proposal: "node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/present-proposal.mjs --creative-draft-file /abs/draft.json --request-summary \"request summary\"",
          recommend_plan: "node $RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/recommend-plan.mjs --creative-draft-file /abs/draft.json --request-summary \"request summary\""
        }
      });
      return;
    }
    ...
    if (request.method === "POST" && url.pathname.startsWith("/api/plans/") && url.pathname.endsWith("/approve")) {
      ...
      const approvedPlan = withStore((store) => {
        ...
        store.appendEvent("plan_updated", {
          ...
          message: requiresTerminalFlow
            ? localize(savedPlan.preferred_language, `대시보드 승인 완료. 현재 RufusAgent 터미널 세션에 승인이라고 입력해 주세요. ${savedPlan.dashboard_url}`, `Dashboard approval recorded. Type 승인 in the current RufusAgent terminal session. ${savedPlan.dashboard_url}`)
      ...
      const approvalMessage = isStructurallyMultiAgentPlan(approvedPlan)
        ? localize(approvedPlan.preferred_language, `대시보드 승인 완료. 현재 RufusAgent 터미널 세션에 승인이라고 입력해 주세요. ${approvedPlan.dashboard_url}`, `Dashboard approval recorded. Type 승인 in the current RufusAgent terminal session. ${approvedPlan.dashboard_url}`)
    ...
    if (request.method === "POST" && url.pathname.startsWith("/api/plans/") && url.pathname.endsWith("/confirm-terminal")) {
      ...
      sendJson(response, 405, {
        error: "terminal confirmation must happen in the active RufusAgent terminal session",
        hint: localize(plan?.preferred_language, "현재 RufusAgent 터미널 세션에 승인이라고 입력하세요.", "Type `승인` in the current RufusAgent terminal session.")
      });
      return;
    }
    ...
    if (request.method === "POST" && url.pathname === "/api/runs") {
      ...
      if (isStructurallyMultiAgentPlan(plan)) {
        sendJson(response, 405, {
          ...
          hint: localize(plan.preferred_language, "대시보드 승인 후 현재 RufusAgent 터미널 세션에 승인이라고 입력하세요.", "After dashboard approval, type `승인` in the current RufusAgent terminal session.")
        });
        return;
      }
    ...
    if (request.method === "POST" && url.pathname.startsWith("/api/runs/") && url.pathname.endsWith("/retry")) {
      ...
      if (isStructurallyMultiAgentPlan(plan)) {
        sendJson(response, 405, {
          ...
          hint: localize(plan.preferred_language, "대시보드에서 다시 승인한 뒤 현재 RufusAgent 터미널 세션에 승인이라고 입력하세요.", "Approve again in the dashboard, then type `승인` in the current RufusAgent terminal session.")
        });
        return;
      }
...
httpServer.listen(PORT, HOST, () => {
  console.log(`rufusagent dashboard listening on ${DASHBOARD_URL}`);
