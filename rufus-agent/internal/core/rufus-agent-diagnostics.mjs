import { listStructuredLogs } from "./rufus-agent-log.mjs";
import { collectAdapterStatusForRun } from "./rufus-agent-adapters.mjs";
...
function collectForeignRunReports(run, agentId) {
  ...
  const runtimeLogs = listStructuredLogs("rufus-agent-runtime", { limit: 400 });
...
export function collectRunDiagnostics(store, runId, { logLimit = 60 } = {}) {
  ...
  if (!run) {
    return {
      ...
      recent_logs: listStructuredLogs("rufus-agent-runtime", { runId, limit: logLimit })
    };
  }
