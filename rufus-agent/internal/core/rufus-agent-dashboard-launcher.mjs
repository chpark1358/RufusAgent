...
function dashboardLaunchEnv({ workspaceRoot, dbPath, runtimeRoot, dashboardUrl }) {
  return {
    ...process.env,
    RUFUS_AGENT_WORKSPACE_ROOT: workspaceRoot,
    ...(dbPath ? { RUFUS_AGENT_DB_PATH: dbPath } : {}),
    ...(runtimeRoot ? { RUFUS_AGENT_RUNTIME_ROOT: runtimeRoot } : {}),
    RUFUS_AGENT_DASHBOARD_URL: dashboardUrl
  };
}
...
function defaultStrategies() {
  ...
  if (interactive || process.env.RUFUS_AGENT_ALLOW_VOLATILE_DASHBOARD_BACKENDS === "1") {
...
function resolveStrategyOrder() {
  const explicit = String(process.env.RUFUS_AGENT_DASHBOARD_DAEMON_STRATEGY ?? "").trim();
...
function schedulerTaskName(workspaceRoot, dashboardUrl) {
  return `RufusAgentDashboard_${stableHash(`${workspaceRoot}|${dashboardUrl}`).slice(0, 12)}`;
}
...
async function launchViaWindowsSchtasks(options) {
  ...
  const wslUser = process.env.RUFUS_AGENT_WSL_RELAY_USER || process.env.USER || userInfo().username || "root";
...
async function launchViaSystemdRun(options) {
  ...
  const { wrapperPath } = ensureWrapperScript(options);
  const unitName = `rufus-agent-dashboard-${stableHash(`${options.workspaceRoot}|${options.dashboardUrl}`).slice(0, 12)}`;
...
async function launchViaTmux(options) {
  ...
  const { wrapperPath } = ensureWrapperScript(options);
  const sessionName = `rufusagent_dash_${stableHash(`${options.workspaceRoot}|${options.dashboardUrl}`).slice(0, 10)}`;
