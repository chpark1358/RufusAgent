...
import { openStoreWithOptions } from "./rufus-agent-store.mjs";
...
export function resolveRufusAgentBinding(options = {}) {
  ...
  const envWorkspaceRoot = env.RUFUS_AGENT_WORKSPACE_ROOT ? resolve(env.RUFUS_AGENT_WORKSPACE_ROOT) : null;
  const envDbPath = env.RUFUS_AGENT_DB_PATH ? resolve(env.RUFUS_AGENT_DB_PATH) : null;
  const envRuntimeRoot = env.RUFUS_AGENT_RUNTIME_ROOT ? resolve(env.RUFUS_AGENT_RUNTIME_ROOT) : null;
  const installRoot = env.RUFUS_AGENT_INSTALL_ROOT ? resolve(env.RUFUS_AGENT_INSTALL_ROOT) : rufusagentInstallRoot();
  ...
  let source = {
    ...
    install_root: env.RUFUS_AGENT_INSTALL_ROOT ? "env" : "detected"
  };
