...
import { resolveRufusAgentBinding } from "./rufus-agent-binding-resolver.mjs";
import { deriveRemoteDashboardBaseUrlFromPlannerUrl } from "./rufus-agent-public-remote-config.mjs";
...
function resolveBridgeIdentity(options = {}) {
  const binding = resolveRufusAgentBinding({
    workspaceRoot: options.workspaceRoot ?? process.env.RUFUS_AGENT_WORKSPACE_ROOT ?? process.cwd(),
    dbPath: options.dbPath ?? process.env.RUFUS_AGENT_DB_PATH ?? null,
    runtimeRoot: options.runtimeRoot ?? process.env.RUFUS_AGENT_RUNTIME_ROOT ?? null,
...
export function remoteDashboardBaseUrl(options = {}) {
  ...
  const explicitBaseUrl = normalizeBaseUrl(
    ...
    ?? env.RUFUS_AGENT_REMOTE_DASHBOARD_BASE_URL
    ?? ""
  );
  if (explicitBaseUrl) {
...
export function dashboardBridgeTokenPath(options = {}) {
  const runtimeRoot = resolve(options.runtimeRoot ?? process.env.RUFUS_AGENT_RUNTIME_ROOT ?? process.cwd());
...
export function dashboardBridgeTokenTtlMs(options = {}) {
  ...
  return resolvePositiveInteger(
    ...
    ?? env.RUFUS_AGENT_DASHBOARD_BRIDGE_TOKEN_TTL_MS,
    30 * 60 * 1000
  );
}
