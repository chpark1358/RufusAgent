...
import { appendStructuredLog } from "./rufus-agent-log.mjs";
import { detectPreferredLanguage, localize, runtimeDefaultPhrases } from "./rufus-agent-language.mjs";
import { RUFUS_AGENT_PLANNER_BACKEND, RUFUS_AGENT_PLANNER_DRIVER } from "./rufus-agent-planner-driver.mjs";
import {
  ...
  workspaceScopeId
} from "./rufus-agent-runtime-paths.mjs";
...
function normalizeRuntimeSkillName(value) {
  return String(value ?? "")
    .replaceAll("rufus-agent-runtime-codex", "rufus-agent-runtime")
    .replaceAll("rufus-agent-runtime@1.0.0", "rufus-agent-runtime-reporting@1.0.0")
    .replaceAll("rufus-agent-runtime-codex@1.0.0", "rufus-agent-runtime-reporting@1.0.0");
}
...
function hydratePlanRecord(plan, fallbackWorkspace = null) {
  ...
  const normalizeLegacyPlannerMetadata = Boolean(planningBackend && planningBackend !== RUFUS_AGENT_PLANNER_BACKEND && !planningDriver);
  const normalizedPlanningBackend = normalizeLegacyPlannerMetadata
    ? RUFUS_AGENT_PLANNER_BACKEND
  ...
  return {
    ...
    agents: (plan.agents ?? []).map((agent) => {
      ...
      return {
        ...
        reporting_contract: normalizeRuntimeSkillName(agent.reporting_contract ?? "rufus-agent-runtime-reporting@1.0.0"),
...
export function openStoreWithOptions(dbPath = resolveDefaultDbPath(), options = {}) {
  ...
  function createPlannerJob(payload = {}) {
    ...
    const job = {
      ...
      planner_driver: normalizeText(payload.planner_driver, RUFUS_AGENT_PLANNER_DRIVER),
  ...
  function savePlan(plan, saveOptions = {}) {
    try {
      return withImmediateTransaction(() => {
        ...
        validateBySchemaId("https://rufusagent.local/schemas/plan.schema.json", data, "plan");
  ...
  function saveRun(run, saveOptions = {}) {
    try {
      return withImmediateTransaction(() => {
        ...
        validateBySchemaId("https://rufusagent.local/schemas/run.schema.json", data, "run");
