...
export function resolveCreativeDraftSource(options = {}) {
  ...
  const draftFile = normalizeText(options.creativeDraftFile ?? env.RUFUS_AGENT_CREATIVE_DRAFT_FILE);
  const draftBase64 = normalizeText(options.creativeDraftBase64 ?? env.RUFUS_AGENT_CREATIVE_DRAFT_BASE64);
  const draftJson = normalizeText(options.creativeDraftJson ?? env.RUFUS_AGENT_CREATIVE_DRAFT_JSON);
...
export function creativeDraftGuidance() {
  return {
    ...
    allowed_sources: [
      ...
      "RUFUS_AGENT_CREATIVE_DRAFT_FILE=/abs/path/to/draft.json"
    ],
    rules: [
