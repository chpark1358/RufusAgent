import {
  ...
  localize
} from "./rufus-agent-language.mjs";
import {
  REMOTE_PLANNER_DRIVER,
  resolvePlannerConfig,
  resolvePlannerModel,
  validatePlannerConfig
} from "./rufus-agent-planner-driver.mjs";
import { requestRemotePlannerDraft } from "./rufus-agent-remote-planner.mjs";
import { loadCreativeDraft } from "./rufus-agent-creative-draft.mjs";
import { dashboardUrlForWorkspace, getWorkspaceContext } from "./rufus-agent-runtime-paths.mjs";
import { validatePlan } from "./rufus-agent-validation.mjs";
...
function materializeRemotePlanTemplate(requestSummary, remotePlan, options = {}) {
  ...
  const authoritativeDbPath = options.authoritativeDbPath ?? process.env.RUFUS_AGENT_DB_PATH ?? "";
