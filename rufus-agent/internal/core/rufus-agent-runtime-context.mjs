...
import { localize, preferredLanguageName, runtimeDefaultPhrases } from "./rufus-agent-language.mjs";
import { defaultRuntimeRoot, skillScriptPath, rufusagentInstallRoot } from "./rufus-agent-runtime-paths.mjs";
...
function defaultWindowsInstallRoot() {
  const explicit = String(
    process.env.RUFUS_AGENT_WINDOWS_INSTALL_ROOT
    ?? process.env.RUFUS_AGENT_CMD_INSTALL_ROOT
    ?? ""
  ).trim();
  if (explicit) {
    return toWindowsPath(explicit);
  }
  const installRoot = String(rufusagentInstallRoot() ?? "").trim();
  if (process.platform === "win32") {
...
function buildPosixCommandScript(context) {
  ...
  if (context.isMainAgent) {
    return [
      "#!/usr/bin/env bash",
      "set -euo pipefail",
      `RUFUS_AGENT_INSTALL_ROOT_DEFAULT=${JSON.stringify(rufusagentInstallRoot())}`,
      "RUFUS_AGENT_INSTALL_ROOT=\"${RUFUS_AGENT_INSTALL_ROOT:-$RUFUS_AGENT_INSTALL_ROOT_DEFAULT}\"",
      ...
      "REPORT_MAIN=\"$RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent/scripts/report-main.mjs\"",
      "",
  ...
  return [
    ...
    `AGENT_ID=${JSON.stringify(agentId)}`,
    `TASK_ID=${JSON.stringify(taskId)}`,
    "REPORT_PROGRESS=\"$RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent-runtime/scripts/report-progress.mjs\"",
    "REPORT_ARTIFACT=\"$RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent-runtime/scripts/report-artifact.mjs\"",
    "REPORT_SESSION=\"$RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent-runtime/scripts/report-session.mjs\"",
    "REPORT_COMPLETION=\"$RUFUS_AGENT_INSTALL_ROOT/skills/rufus-agent-runtime/scripts/report-completion.mjs\"",
    "",
    "# This launcher is generated for the current install root.",
    `# Preferred user language: ${language}`,
    "",
...
function buildCmdCommandScript(context) {
  ...
  if (context.isMainAgent) {
    const lines = [
      "@echo off",
      "setlocal",
      `set "RUFUS_AGENT_INSTALL_ROOT_DEFAULT=${defaultWindowsInstallRoot()}"`,
      "if not defined RUFUS_AGENT_INSTALL_ROOT if defined RUFUS_AGENT_INSTALL_ROOT_DEFAULT set \"RUFUS_AGENT_INSTALL_ROOT=%RUFUS_AGENT_INSTALL_ROOT_DEFAULT%\"",
      `set "DB_PATH=${dbPath}"`,
      `set "PLAN_ID=${planId}"`,
      `set "RUN_ID=${runId}"`,
      `set "AGENT_ID=${agentId}"`,
      `set "TASK_ID=${taskId}"`,
      "set \"REPORT_MAIN=%RUFUS_AGENT_INSTALL_ROOT%\\skills\\rufus-agent\\scripts\\report-main.mjs\"",
      "",
      "REM Start report",
      buildMainReportCommandLine({
        executable: "\"%REPORT_MAIN%\"",
  ...
  const lines = [
    ...
    `set "AGENT_ID=${agentId}"`,
    `set "TASK_ID=${taskId}"`,
    "set \"REPORT_PROGRESS=%RUFUS_AGENT_INSTALL_ROOT%\\skills\\rufus-agent-runtime\\scripts\\report-progress.mjs\"",
    "set \"REPORT_ARTIFACT=%RUFUS_AGENT_INSTALL_ROOT%\\skills\\rufus-agent-runtime\\scripts\\report-artifact.mjs\"",
    "set \"REPORT_SESSION=%RUFUS_AGENT_INSTALL_ROOT%\\skills\\rufus-agent-runtime\\scripts\\report-session.mjs\"",
    "set \"REPORT_COMPLETION=%RUFUS_AGENT_INSTALL_ROOT%\\skills\\rufus-agent-runtime\\scripts\\report-completion.mjs\"",
    "",
    "REM Start report",
    taskId
...
function buildPowerShellCommandScript(context) {
  ...
  if (context.isMainAgent) {
    const lines = [
      ...
      `$RUFUS_AGENT_INSTALL_ROOT_DEFAULT = ${JSON.stringify(defaultWindowsInstallRoot())}`,
      "if (-not $env:RUFUS_AGENT_INSTALL_ROOT -and $RUFUS_AGENT_INSTALL_ROOT_DEFAULT) { $env:RUFUS_AGENT_INSTALL_ROOT = $RUFUS_AGENT_INSTALL_ROOT_DEFAULT }",
      `$DB_PATH = ${JSON.stringify(dbPath)}`,
      `$PLAN_ID = ${JSON.stringify(planId)}`,
      `$RUN_ID = ${JSON.stringify(runId)}`,
      `$AGENT_ID = ${JSON.stringify(agentId)}`,
      `$TASK_ID = ${JSON.stringify(taskId)}`,
      "$REPORT_MAIN = Join-Path $env:RUFUS_AGENT_INSTALL_ROOT \"skills\\rufus-agent\\scripts\\report-main.mjs\"",
      "",
      "# Start report",
      buildMainReportCommandLine({
        executable: "$REPORT_MAIN",
  ...
  const lines = [
    ...
    `$AGENT_ID = ${JSON.stringify(agentId)}`,
    `$TASK_ID = ${JSON.stringify(taskId)}`,
    "$REPORT_PROGRESS = Join-Path $env:RUFUS_AGENT_INSTALL_ROOT \"skills\\rufus-agent-runtime\\scripts\\report-progress.mjs\"",
    "$REPORT_ARTIFACT = Join-Path $env:RUFUS_AGENT_INSTALL_ROOT \"skills\\rufus-agent-runtime\\scripts\\report-artifact.mjs\"",
    "$REPORT_SESSION = Join-Path $env:RUFUS_AGENT_INSTALL_ROOT \"skills\\rufus-agent-runtime\\scripts\\report-session.mjs\"",
    "$REPORT_COMPLETION = Join-Path $env:RUFUS_AGENT_INSTALL_ROOT \"skills\\rufus-agent-runtime\\scripts\\report-completion.mjs\"",
    "",
    "# Start report",
    taskId
      ? `node $REPORT_PROGRESS --db $DB_PATH --plan-id $PLAN_ID --run-id $RUN_ID --agent-id $AGENT_ID --task-id $TASK_ID --step-index 1 --total-steps 3 --state running --message ${JSON.stringify(phrases.startMessage)} --detail ${JSON.stringify(phrases.startDetail)}`
...
export function prepareRunContextFiles(store, runId, options = {}) {
  ...
  const progressScript = skillScriptPath("rufus-agent-runtime", "report-progress.mjs");
  const artifactScript = skillScriptPath("rufus-agent-runtime", "report-artifact.mjs");
  const sessionScript = skillScriptPath("rufus-agent-runtime", "report-session.mjs");
  const completionScript = skillScriptPath("rufus-agent-runtime", "report-completion.mjs");
  const ingestJsonScript = skillScriptPath("rufus-agent-runtime", "ingest-json-report.mjs");
  ...
  const sharedInstructionLines = [
    "Use the generated RufusAgent runtime reporting transport.",
