...
import { getWorkspaceContext } from "./rufus-agent-runtime-paths.mjs";
import { inferAdapterConfigForWorkspace } from "./rufus-agent-provider-session.mjs";
import {
...
const DEFAULT_REMOTE_CLIENT_NAME = "rufus-agent-community-edition";
...
function defaultHeaders(config, requestId) {
  return {
    ...
    "x-rufus-agent-client-name": config.client_name,
    "x-rufus-agent-protocol-version": config.protocol_version,
    "x-rufus-agent-request-id": requestId
  };
}
...
export function resolveRemotePlannerConfig(options = {}) {
  ...
  const sendWorkspaceRoot = isTruthy(
    ...
    ?? env.RUFUS_AGENT_REMOTE_PLAN_NORMALIZER_SEND_WORKSPACE_ROOT
    ?? env.RUFUS_AGENT_REMOTE_PLANNER_SEND_WORKSPACE_ROOT
  );
  ...
  if (plannerUrl && ticketUrl) {
    ...
    if (plannerOrigin !== ticketOrigin) {
      throw new Error("RUFUS_AGENT_REMOTE_PLAN_NORMALIZER_URL and RUFUS_AGENT_REMOTE_PLAN_NORMALIZER_TICKET_URL must share the same origin");
    }
  }
  return {
    ...
    timeout_ms: resolvePositiveInteger(
      ...
      ?? options.timeoutMs
      ?? env.RUFUS_AGENT_REMOTE_PLAN_NORMALIZER_TIMEOUT_MS
      ?? env.RUFUS_AGENT_REMOTE_PLANNER_TIMEOUT_MS,
      DEFAULT_REMOTE_TIMEOUT_MS
    ),
    client_name: normalizeText(
      options.remotePlannerClientName
      ?? options.remoteNormalizerClientName
      ?? env.RUFUS_AGENT_REMOTE_PLAN_NORMALIZER_CLIENT_NAME
      ?? env.RUFUS_AGENT_REMOTE_PLANNER_CLIENT_NAME,
    ...
    protocol_version: normalizeText(
      ...
      ?? env.RUFUS_AGENT_REMOTE_PLAN_NORMALIZER_PROTOCOL_VERSION
      ?? env.RUFUS_AGENT_REMOTE_PLANNER_PROTOCOL_VERSION,
...
export async function requestRemotePlannerDraft(requestSummary, options = {}) {
  ...
  if (!config.planner_url) {
    throw new Error("remote plan normalizer client requires RUFUS_AGENT_REMOTE_PLAN_NORMALIZER_URL");
  }
  if (!config.ticket_url && !config.allow_unsigned) {
    throw new Error("remote plan normalizer client requires RUFUS_AGENT_REMOTE_PLAN_NORMALIZER_TICKET_URL unless RUFUS_AGENT_REMOTE_PLAN_NORMALIZER_ALLOW_UNSIGNED=true");
  }
  ...
  const extraHeaders = ticket?.token
    ? { "x-rufus-agent-job-ticket": ticket.token }
    : {};
