import { preferredLanguageName, runtimeDefaultPhrases } from "./rufus-agent-language.mjs";
import { skillScriptPath } from "./rufus-agent-runtime-paths.mjs";
...
function mergedSkillsConfig(agent) {
  ...
  return Array.from(new Set([
    ...base,
    ...(isMainAgent(agent) ? [] : ["rufus-agent-runtime"])
  ].filter(Boolean)));
}
...
function buildMainReportCommand({
  ...
  const reportMainScript = skillScriptPath("rufus-agent", "report-main.mjs");
  const args = [
...
export function buildSubagentPacket(plan, run, agent, options = {}) {
  ...
  const prompt = [
    `You are participating in RufusAgent run ${run.run_id}.`,
  ...
  const delegationPrompt = [
    `You are ${agent.agent_id} for RufusAgent run ${run.run_id}.`,
    ...
    `Sandbox, MCP, skills, and path restrictions are RufusAgent coordination policy unless the host launch surface enforces them natively.`,
    ...
    mainAgent
      ? `Use report-main.mjs only. Do not use generic rufus-agent-runtime report-* scripts or JSON ingest for agent_main.`
  ...
  const assistantLaunchRecipe = {
    ...
    notes: adapterProvider === "codex"
      ? [
          ...
          "The launch recipe can pin model, reasoning, and fork_context. Sandbox, MCP, skills, and path restrictions remain RufusAgent policy unless the host surface enforces them natively.",
          "Immediately after spawn_agent returns, run launch_ack_command so RufusAgent knows the agent was actually launched.",
      ...
      : [
          ...
          "Sandbox, MCP, skills, and path restrictions remain RufusAgent policy unless the host surface enforces them natively.",
          "Immediately after manual launch, run launch_ack_command so RufusAgent knows the agent was actually launched.",
          "Do not rewrite the role prompt.",
          "Do not drop the embedded report-* commands.",
          "Do not run launch_ack_command inside the child thread.",
          "Sub-agents must never call report-main.mjs."
  ...
  return {
    ...
    reporting_transport: {
      ...
      helper_skill: mainAgent ? "rufus-agent" : "rufus-agent-runtime",
