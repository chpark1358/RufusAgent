import { buildSubagentPacket as buildBaseSubagentPacket } from "./rufus-agent-bridge.mjs";
import { prepareRunContextFiles } from "./rufus-agent-runtime-context.mjs";
...
export const claudeCodeAdapter = {
  ...
  buildSubagentPacket(plan, run, agent, options = {}) {
    ...
    return {
      ...
      reporting_transport: {
        ...
        helper_skill: "rufus-agent-runtime",
      ...
      adapter_guidance: {
        ...
        note: "RufusAgent targets Claude Code sidechain subagents through a manual manifest flow and does not call a Claude launch API directly. If a Claude Code subagent finishes but its RufusAgent completion report is missing, report-main can reconcile from a Claude session file or session id."
      },
      prompt: base.prompt,
  ...
  buildLaunchManifest(store, runId, options = {}) {
    ...
    return {
      ...
      note: "This RufusAgent adapter targets Claude Code sidechain subagents only and prepares a manual launch manifest rather than calling a scriptable launch API. Missing completion reports can be reconciled from Claude session data.",
      candidates: packets
    };
  },
  launch(store, runId, options = {}) {
    ...
    return {
      ...
      note: "Use the generated manifest to launch Claude Code sidechain subagents manually in the current conversation. If a Claude Code subagent later finishes without writing RufusAgent completion, report-main can reconcile from a Claude session file or session id.",
      provider_refs: {
  ...
  cancel(store, runId) {
    ...
    return {
      ...
      note: "Claude Code cancellation is not wired to a public scriptable API in this RufusAgent adapter. Stop the run locally and halt delegated work manually."
    };
  },
