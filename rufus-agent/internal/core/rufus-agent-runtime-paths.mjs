...
function existingLegacyWorkspaceDataDir(workspaceRoot) {
  const dataDir = resolve(workspaceRoot, ".rufusagent", "data");
  const dbPath = resolve(dataDir, "rufus-agent.sqlite");
  return pathExists(dbPath) ? dataDir : null;
}

function existingLegacyWorkspaceRuntimeRoot(workspaceRoot) {
  const runtimeRoot = resolve(workspaceRoot, ".rufusagent", "runtime");
  return pathExists(runtimeRoot) ? runtimeRoot : null;
}

export function detectWorkspaceRoot(startDir = process.cwd()) {
  const explicitWorkspaceRoot = process.env.RUFUS_AGENT_WORKSPACE_ROOT;
  ...
  while (true) {
    if (pathExists(resolve(currentDir, ".rufusagent"))) {
      return currentDir;
    }
...
export function workspaceContextForDbPath(dbPath, options = {}) {
  ...
  const localMarker = "/.rufusagent/data/rufus-agent.sqlite";
  ...
  const match = resolvedDbPath.match(/[/\\]workspaces[/\\](ws_[^/\\]+)[/\\]data[/\\]rufus-agent\.sqlite$/);
  if (match) {
...
function defaultGlobalRufusAgentRoot() {
  const explicitHome = process.env.RUFUS_AGENT_HOME_DIR;
  if (explicitHome) {
  ...
  const xdgStateHome = process.env.XDG_STATE_HOME
    ? resolve(process.env.XDG_STATE_HOME, "rufusagent")
    : null;
  const xdgDataHome = process.env.XDG_DATA_HOME
    ? resolve(process.env.XDG_DATA_HOME, "rufusagent")
    : null;
  const candidates = unique([
    xdgStateHome,
    xdgDataHome,
    resolve(homedir(), ".local", "state", "rufusagent"),
    resolve(homedir(), ".codex", "rufusagent"),
    resolve(tmpdir(), "rufus-agent-runtime")
  ]);
  ...
  throw new Error("Could not find a writable global RufusAgent home directory.");
}

export function rufusagentInstallRoot() {
  return ROOT_DIR;
}
...
export function defaultDataDir(options = {}) {
  const explicitDataDir = process.env.RUFUS_AGENT_DATA_DIR;
  if (explicitDataDir) {
  ...
  const globalRoot = defaultGlobalRufusAgentRoot();
  const candidateRoots = unique([
    normalizeWorkspacePath(resolve(workspace.workspace_root, ".rufusagent")),
    ...windowsMountCaseVariants(resolve(workspace.workspace_root, ".rufusagent")),
    ...
    normalizeWorkspacePath(resolve(tmpdir(), "rufus-agent-runtime", "workspaces", workspace.workspace_id))
  ]);
  ...
  throw new Error("Could not find a writable RufusAgent data directory.");
}

export function defaultDbPath(options = {}) {
  if (process.env.RUFUS_AGENT_DB_PATH) {
    return resolve(process.env.RUFUS_AGENT_DB_PATH);
  }

  return resolve(defaultDataDir(options), "rufus-agent.sqlite");
}

export function defaultLogDir(options = {}) {
  if (process.env.RUFUS_AGENT_LOG_DIR) {
    const writable = tryWritableDir(resolve(process.env.RUFUS_AGENT_LOG_DIR));
    if (writable) {
      return writable;
    }
  }
...
export function defaultRuntimeRoot(options = {}) {
  if (process.env.RUFUS_AGENT_RUNTIME_ROOT) {
    const writable = tryWritableDir(resolve(process.env.RUFUS_AGENT_RUNTIME_ROOT));
    if (writable) {
      return writable;
    }
  }
  ...
  const globalRoot = defaultGlobalRufusAgentRoot();
  const candidateRoots = unique([
    normalizeWorkspacePath(resolve(workspace.workspace_root, ".rufusagent", "runtime")),
    ...windowsMountCaseVariants(resolve(workspace.workspace_root, ".rufusagent", "runtime")),
  ...
  throw new Error("Could not find a writable RufusAgent runtime directory.");
}

export function dashboardHost(options = {}) {
  return String(
    options.host
    ?? (options.preferEnv !== false ? process.env.RUFUS_AGENT_DASHBOARD_HOST : null)
    ?? "127.0.0.1"
  );
}
...
export function dashboardPortForWorkspace(options = {}) {
  ...
  if (options.preferEnv !== false && process.env.RUFUS_AGENT_DASHBOARD_PORT) {
    return resolvePositiveInteger(process.env.RUFUS_AGENT_DASHBOARD_PORT, 18081);
  }

  const workspace = getWorkspaceContext(options);
  const basePort = resolvePositiveInteger(
    options.basePort ?? process.env.RUFUS_AGENT_DASHBOARD_PORT_BASE,
    18081
  );
  const span = resolvePositiveInteger(
    options.portSpan ?? process.env.RUFUS_AGENT_DASHBOARD_PORT_SPAN,
    20000
  );
...
export function dashboardUrlForWorkspace(options = {}) {
  ...
  if (options.preferEnv !== false && process.env.RUFUS_AGENT_DASHBOARD_URL) {
    return String(process.env.RUFUS_AGENT_DASHBOARD_URL);
  }
