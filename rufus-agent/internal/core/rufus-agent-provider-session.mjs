...
import { CLAUDE_ADAPTER_BACKEND, CLAUDE_ADAPTER_TYPE } from "./rufus-agent-claude-code-adapter.mjs";
import { CODEX_ADAPTER_BACKEND, CODEX_ADAPTER_TYPE } from "./rufus-agent-codex-adapter.mjs";
...
export function hasExplicitAdapterConfigEnv() {
  return Boolean(process.env.RUFUS_AGENT_DEFAULT_ADAPTER_TYPE || process.env.RUFUS_AGENT_DEFAULT_ADAPTER_BACKEND);
}
...
export function findCodexSessionTranscriptPath(threadId) {
  ...
  const codexSessionsRoot = resolve(process.env.RUFUS_AGENT_CODEX_SESSIONS_ROOT ?? resolve(homedir(), ".codex", "sessions"));
...
export function inferAdapterConfigForWorkspace({ workspaceRoot = null } = {}) {
  const codexThreadId = process.env.RUFUS_AGENT_MAIN_PROVIDER_SESSION_THREAD_ID ?? process.env.CODEX_THREAD_ID ?? null;
  if (codexThreadId) {
    return {
  ...
  const explicitSessionId = process.env.RUFUS_AGENT_MAIN_PROVIDER_SESSION_ID ?? process.env.CLAUDE_SESSION_ID ?? null;
  const explicitSessionFile = process.env.RUFUS_AGENT_MAIN_PROVIDER_SESSION_FILE ?? null;
...
export function resolveCodexMainProviderSessionIdentity({ workspaceRoot = null } = {}) {
  const threadId = process.env.RUFUS_AGENT_MAIN_PROVIDER_SESSION_THREAD_ID ?? process.env.CODEX_THREAD_ID ?? null;
  if (!threadId) {
    const inferred = inferAdapterConfigForWorkspace({ workspaceRoot });
    if (inferred?.adapter_type === CLAUDE_ADAPTER_TYPE) {
      throw new Error(
        "Cannot determine Codex main session identity. CODEX_THREAD_ID is missing in the current terminal session. This workspace currently looks like Claude Code. Re-run with --adapter-type claude-code-subagent --adapter-backend claude-code-manual or set RUFUS_AGENT_DEFAULT_ADAPTER_TYPE/RUFUS_AGENT_DEFAULT_ADAPTER_BACKEND."
      );
    }
...
export function resolveClaudeMainProviderSessionIdentity(workspaceRoot) {
  ...
  if (!sessionId && !transcriptPath) {
    throw new Error(
      "Cannot determine Claude main session identity. Set RUFUS_AGENT_MAIN_PROVIDER_SESSION_ID or RUFUS_AGENT_MAIN_PROVIDER_SESSION_FILE before launching the run."
    );
  }
  return {
